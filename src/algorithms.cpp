...
template<typename T>
alphatensor::AlphaTensorAlgorithm<T>::AlphaTensorAlgorithm(const std::string& variant) : variant_(variant) {}

template<typename T>
typename alphatensor::AlphaTensorAlgorithm<T>::MatrixType alphatensor::AlphaTensorAlgorithm<T>::multiply(const MatrixType& A, const MatrixType& B) const {
    if (!can_handle(A.rows(), A.cols(), B.rows(), B.cols())) {
    ...
    return StrassenAlgorithm<T>().multiply(A, B);
}

template<typename T>
std::string alphatensor::AlphaTensorAlgorithm<T>::name() const { return "AlphaTensor-" + variant_; }

template<typename T>
bool alphatensor::AlphaTensorAlgorithm<T>::can_handle(size_type A_rows, size_type A_cols,
                                     size_type B_rows, size_type B_cols) const {
    return A_cols == B_rows && A_rows == A_cols && B_rows == B_cols;
}

template<typename T>
typename alphatensor::AlphaTensorAlgorithm<T>::MatrixType alphatensor::AlphaTensorAlgorithm<T>::alpha_tensor_4x4(const MatrixType& A, const MatrixType& B) const {
    ...
    MatrixType A11 = A.submatrix(0, 0, 2, 2);
    MatrixType A12 = A.submatrix(0, 2, 2, 4);
    MatrixType A21 = A.submatrix(2, 0, 4, 2);
    MatrixType A22 = A.submatrix(2, 2, 4, 4);
    
    MatrixType B11 = B.submatrix(0, 0, 2, 2);
    MatrixType B12 = B.submatrix(0, 2, 2, 4);
    MatrixType B21 = B.submatrix(2, 0, 4, 2);
    MatrixType B22 = B.submatrix(2, 2, 4, 4);
    
    // Strassen's seven multiplications
    MatrixType P1 = alpha_tensor_2x2(A11, B12 - B22);
    MatrixType P2 = alpha_tensor_2x2(A11 + A12, B22);
    MatrixType P3 = alpha_tensor_2x2(A21 + A22, B11);
    MatrixType P4 = alpha_tensor_2x2(A22, B21 - B11);
    MatrixType P5 = alpha_tensor_2x2(A11 + A22, B11 + B22);
    MatrixType P6 = alpha_tensor_2x2(A12 - A22, B21 + B22);
    MatrixType P7 = alpha_tensor_2x2(A11 - A21, B11 + B12);
...
template<typename T>
typename alphatensor::AlphaTensorAlgorithm<T>::MatrixType alphatensor::AlphaTensorAlgorithm<T>::alpha_tensor_2x2(const MatrixType& A, const MatrixType& B) const {
    MatrixType result(2, 2);
...
template<typename T>
typename alphatensor::AlphaTensorAlgorithm<T>::Factorization alphatensor::AlphaTensorAlgorithm<T>::get_factorization() const {
